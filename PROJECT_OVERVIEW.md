...
- **A brand's `quantity_available` / `bookable` / `amenities` are all blank** → its `rooms`
  ...
  **Cloudflare IP-reputation blocking of the GitHub-runner egress** on that brand's frontend
  ...
  The *real* fix is non-datacenter egress: a residential/mobile **proxy** for the enrichment
  fetches or a **self-hosted runner**. Two safety nets already in place (both 2026-07-07):
